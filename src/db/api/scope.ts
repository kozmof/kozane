...
import type { WithDB, Scope } from "./types";
...
type GetScope = WithDB & { scopeId: string };
...
export async function addScope({ db }: WithDB): Promise<string> {
  const [row] = await db.insert(scopeTable).values({}).returning({ id: scopeTable.id });
  return row.id;
}

type DeleteScope = WithDB & { scopeId: string };
