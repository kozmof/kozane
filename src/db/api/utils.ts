 ...
 * does not, but either way is not this namespace's to act on. The distinction between the two
 * is deliberately not drawn — this workspace's own CLI lists and moves cards across
 * namespaces, so "it is elsewhere" and "it is nowhere" are the same answer to the only
 ...
 * limit: `getCardPartitionNames` chunked, `loadCards` in the CLI chunked, and `cardsInNamespace`
 ...
 * short of the namespace — the same reason `loadCards` was already batching beside it.
 *
 ...
 * than `chunked`'s row-count default, for the reason `getCardPartitionNames` gave when it was
 ...
 * namespaces, partitions, layers, scopes, taskspaces. HTTP routes check the limit themselves so
