...
describe("WarpPalette", () => {
  it("groups the rows by namespace and marks the current one", () => {
    render(WarpPalette, { props: makeProps() });

    expect(screen.getByText("Kozane (this namespace)")).toBeInTheDocument();
  ...
  it("jumps to a row that is clicked", async () => {
    ...
    expect(onJump).toHaveBeenCalledWith(expect.objectContaining({ id: "w3", namespaceId: "p2" }));
  });
