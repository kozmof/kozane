<script lang="ts">
  ...
  import type { PartitionWithColor } from "$lib/types";
  ...
  let { partitions, partitionId, onChange }: Props = $props();

  let open = $state(false);
  let dropdownEl: HTMLDivElement = $state()!;

  let active = $derived(partitions.find((b) => b.id === partitionId) ?? partitions[0]);
  ...
  {#if open}
    ...
    >
      {#each partitions as b (b.id)}
        {@const isActive = b.id === partitionId}
        <button
