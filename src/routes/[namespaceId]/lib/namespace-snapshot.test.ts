...
import { loadNamespaceSnapshot } from "./namespace-snapshot.js";
...
async function namespace(db: DB, name = "P") {
  ...
  return { namespaceId, partitionId, layerId };
}

const load = (db: DB, namespaceId: string) =>
...
describe("loadNamespaceSnapshot", () => {
  it("answers null for a namespace that does not exist", async () => {
    const db = await createTestDB();
    expect(await load(db, "nope")).toBeNull();
  });

  it("carries the cards of the namespace's partitions, with their glue groups", async () => {
    const db = await createTestDB();
    ...
    const loaded = await load(db, namespaceId);
  ...
  it("carries the scope memberships of this namespace's cards only", async () => {
    const db = await createTestDB();
    const mine = await namespace(db, "mine");
    const theirs = await namespace(db, "theirs");
    const scopeId = await addScope({ db, name: "shared" });
    const cardId = await addCard({ db, partitionId: mine.partitionId, content: "mine" });
    const otherCardId = await addCard({ db, partitionId: theirs.partitionId, content: "theirs" });
    await addScopeRel({ db, scopeId, cardId });
    await addScopeRel({ db, scopeId, cardId: otherCardId });

    const loaded = await load(db, mine.namespaceId);
  ...
  it("redacts taskspace paths when the caller asks it to", async () => {
    ...
    const exported = await loadNamespaceSnapshot({
      db,
      namespaceId,
  ...
  it("omits scopes, scope relations, and taskspaces entirely when the caller asks it to", async () => {
    ...
    const cardId = await addCard({ db, partitionId, content: "one" });
    await addScopeRel({ db, scopeId, cardId });
  ...
  describe("with a taskspace directory on disk", () => {
    ...
    it("never reads taskspace files from disk when includeScopedFiles is false", async () => {
      const db = await createTestDB();
      const { namespaceId } = await namespace(db);
      await addTaskspace({ db, namespaceId, name: "demo", path: "demo" });
    ...
    it("leaves a taskspace that belongs to no scope, and so is never drawn, out of the export entirely", async () => {
      ...
      const { namespaceId } = await namespace(db);
      const scopeId = await addScope({ db, name: "work" });
      const drawn = await addTaskspace({ db, scopeId, namespaceId, name: "drawn", path: "demo" });
      await addTaskspace({ db, namespaceId, name: "unscoped", path: "demo" }); // no `scopeId`
    ...
    it("still names every taskspace to the live board, scope or no scope", async () => {
      const db = await createTestDB();
      const { namespaceId } = await namespace(db);
      await addTaskspace({ db, namespaceId, name: "unscoped", path: "demo" });
    ...
    it("builds no file tree for a taskspace that has no directory, even when asked to", async () => {
      const db = await createTestDB();
      const { namespaceId } = await namespace(db);
      await addTaskspace({ db, namespaceId, name: "no-dir" }); // no `path`
  ...
  it("produces a snapshot the poll's reader accepts unchanged", async () => {
    ...
    const read = readNamespaceSnapshot(JSON.parse(JSON.stringify(loaded!.snapshot)));
