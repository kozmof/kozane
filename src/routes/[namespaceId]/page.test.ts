...
import NamespacePage from "./+page.svelte";
import { SAFE_AREA_GRACE_MS } from "./lib/namespace-page.js";
...
const data = {
  ...
  partitions: [
    { id: "b1", namespaceId: "namespace-1", name: "General", isDefault: true },
    { id: "b2", namespaceId: "namespace-1", name: "Research", isDefault: false },
  ],
  layers: [
    { id: "l1", namespaceId: "namespace-1", name: "Base", position: 0, isDefault: true },
    { id: "l2", namespaceId: "namespace-1", name: "Draft", position: 1, isDefault: false },
  ],
  cards: [
    {
      id: "card-1",
  ...
  warps: [
    ...
    { id: "warp-3", namespaceId: "namespace-1", posX: 1400, posY: 1600 },
  ],
  ...
  // What the other namespaces contribute to the warp palette.
  warpDirectory: [
    {
      id: "warp-9",
...
describe("Namespace page", () => {
  it("uses the configured zoom step", async () => {
    const { container } = render(NamespacePage, {
      props: {
        ...
        params: { namespaceId: "namespace-1" },
        form: null,
      },
    });
  ...
  it("sends back the tag it holds and leaves the board alone when nothing changed", async () => {
    ...
    expect(fetch.mock.calls[1][0]).toBe("/namespace-1/api/snapshot");
  ...
  it("carries a glued partner along with the card under the pointer", async () => {
    ...
    render(NamespacePage, {
      props: { data: glued, params: { namespaceId: "namespace-1" }, form: null },
    });
  ...
  it("glues selected cards through the composed board UI", async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledOnce());
    expect(fetch).toHaveBeenCalledWith("/namespace-1/api/glues", {
      method: "POST",
...
describe("Card width", () => {
  ...
  it("widens the card on the drag and saves where it lands", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith("/namespace-1/api/cards/card-1", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
describe("Layers", () => {
  ...
  it("does not sweep cards on dimmed layers into a rectangle selection", async () => {
    ...
    const { container } = render(NamespacePage, {
      props: { data: spread, params: { namespaceId: "namespace-1" }, form: null },
    });
  ...
  it("creates a card on the selected layer", async () => {
    ...
    expect(url).toBe("/namespace-1/api/cards");
    expect(JSON.parse(init.body)).toMatchObject({ layerId: "l2", content: "Gamma" });
  });

  it("creates the card on the namespace navigated to, not the one left behind", async () => {
    const fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        id: "card-9",
    ...
    vi.stubGlobal("fetch", fetch);
    const { rerender } = render(NamespacePage, {
      props: { data, params: { namespaceId: "namespace-1" }, form: null },
    });
    ...
    const other = {
      ...
      partitions: [{ id: "b3", namespaceId: "namespace-2", name: "General", isDefault: true }],
      layers: [
        { id: "l3", namespaceId: "namespace-2", name: "Base", position: 0, isDefault: true },
      ],
      cards: [],
      warps: [],
      warpDirectory: [],
    };
    await rerender({ data: other, params: { namespaceId: "namespace-2" }, form: null });
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledOnce());
    const [url, init] = fetch.mock.calls[0];
    expect(url).toBe("/namespace-2/api/cards");
    expect(JSON.parse(init.body)).toMatchObject({ partitionId: "b3", layerId: "l3" });
  });
  ...
  it("renames a layer from a double-click on its row", async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledOnce());
    expect(fetch).toHaveBeenCalledWith("/namespace-1/api/layers/l2", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  it("reorders layers by dragging a row and restacks the canvas", async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledOnce());
    expect(fetch).toHaveBeenCalledWith("/namespace-1/api/layers", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  it("moves the selected cards to another layer and follows them there", async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledOnce());
    expect(fetch).toHaveBeenCalledWith("/namespace-1/api/cards/layer", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  it("does not compound the scope dim with the layer dim", async () => {
    ...
    render(NamespacePage, {
      props: { data: scoped, params: { namespaceId: "namespace-1" }, form: null },
    });
  ...
  it("comes back to the layer that was being worked on", async () => {
    const first = render(NamespacePage, {
      props: { data, params: { namespaceId: "namespace-1" }, form: null },
    });
    await fireEvent.mouseEnter(screen.getByLabelText("Layers").parentElement!);
    await fireEvent.click(layerOption("Draft"));
  ...
  it("shows the reason the server gives for a refused reorder", async () => {
    const fetch = vi.fn().mockResolvedValue({
      ok: false,
      json: async () => ({
        message: "The namespace's layers changed elsewhere. Reload to see the current order.",
      }),
    });
    vi.stubGlobal("fetch", fetch);
    render(NamespacePage, {
...
describe("Warps", () => {
  ...
  it("sets a warp under the mouse pointer", async () => {
    const fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ id: "warp-new", namespaceId: "namespace-1", posX: 1200, posY: 850 }),
    });
    vi.stubGlobal("fetch", fetch);
    renderPage({ warps: [] });
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledOnce());
    expect(fetch).toHaveBeenCalledWith("/namespace-1/api/warps", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("wraps round rather than sticking on a warp the board cannot centre", async () => {
    ...
    const { container } = renderPage({
      warps: [
        ...
        { id: "warp-2", namespaceId: "namespace-1", posX: 2700, posY: 1000 },
      ],
    });
    const canvas = canvasOf(container);
  ...
  it("does one thing when one key is bound to two warp actions", async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledOnce());
    expect(fetch.mock.calls[0][0]).toBe("/namespace-1/api/warps");
    expect(screen.getByLabelText("Warp 1")).toHaveAttribute("data-warp-id", "warp-1");
  });
  ...
  describe("palette", () => {
    ...
    it("lists every namespace's warps, this one's first", async () => {
    ...
    it("re-reads the other namespaces' warps when it opens", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith("/namespace-1/api/warp-directory");
      await waitFor(() =>
    ...
    it("keeps the warps it was given when the re-read is not a list of rows", async () => {
      ...
      vi.stubGlobal("fetch", directoryResponse([{ id: "warp-10", namespaceName: "Research" }]));
      renderPage();
    ...
    it("moves the view to a warp of this namespace without navigating", async () => {
      vi.stubGlobal("fetch", directoryResponse(data.warpDirectory));
      const { container } = renderPage();
      const canvas = canvasOf(container);
    ...
    it("navigates to the other namespace, naming the warp it is headed for", async () => {
      vi.stubGlobal("fetch", directoryResponse(data.warpDirectory));
      renderPage();
      ...
      expect(goto).toHaveBeenCalledWith("/namespace-2?warp=warp-9");
    });

    it("removes another namespace's warp and renumbers what is left of it", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith("/namespace-2/api/warps/warp-9", { method: "DELETE" });
      await waitFor(() =>
        expect(screen.queryByRole("option", { name: /Umesao 1969/ })).not.toBeInTheDocument(),
      );
      // What was warp 2 in that namespace is now its warp 1.
      expect(screen.getByRole("option", { name: /second/ })).toHaveTextContent("Warp 1");
    });

    it("puts another namespace's warp back when removing it fails", async () => {
      const fetch = vi
        .fn()
    ...
    it("removes one of this namespace's warps through the same path as the x key", async () => {
      vi.stubGlobal("fetch", directoryResponse(data.warpDirectory));
      renderPage();

      await openPalette();
      await fireEvent.click(screen.getByRole("button", { name: "Remove warp 1 in Namespace" }));
    ...
    it("keeps the trailing slash a static export's pages are served under", async () => {
      ...
      page.url = new URL("http://localhost/namespace-1/") as typeof page.url;
      vi.stubGlobal("fetch", directoryResponse(data.warpDirectory));
      renderPage({ readonly: true });
  ...
  describe("landing from another namespace", () => {
    ...
    afterEach(() => {
      visit("http://localhost/namespace-1");
    });

    it("opens on the warp the url names and highlights it", async () => {
      visit("http://localhost/namespace-1?warp=warp-2");
      vi.stubGlobal("fetch", vi.fn());
      ...
      await waitFor(() => expect(replaceState).toHaveBeenCalledWith("/namespace-1", {}));
    });

    it("opens the next namespace in the middle rather than where the last one was left", async () => {
      vi.stubGlobal("fetch", vi.fn());
      const { container, rerender } = render(NamespacePage, {
        props: { data, params: { namespaceId: "namespace-1" }, form: null },
      });
    ...
    it("drops the warp it landed on from the query and leaves the rest alone", async () => {
      visit("http://localhost/namespace-1?warp=warp-2&scope=reading");
      vi.stubGlobal("fetch", vi.fn());

      renderPage();

      await waitFor(() =>
        expect(replaceState).toHaveBeenCalledWith("/namespace-1?scope=reading", {}),
      );
    });

    it("opens in the middle of the board when the warp is gone", () => {
      visit("http://localhost/namespace-1?warp=removed-elsewhere");
      vi.stubGlobal("fetch", vi.fn());
  ...
  describe("landing from the tag index", () => {
    ...
    it("centres on the card the url names and selects it", async () => {
      visit("http://localhost/namespace-1?card=card-far");
      vi.stubGlobal("fetch", vi.fn());
    ...
    it("drops the card it landed on from the query and leaves the rest alone", async () => {
      visit("http://localhost/namespace-1?card=card-far&scope=reading");
      vi.stubGlobal("fetch", vi.fn());
    ...
    it("opens in the middle of the board when the card is gone", async () => {
      ...
      visit("http://localhost/namespace-1?card=deleted-elsewhere");
      vi.stubGlobal("fetch", vi.fn());
    ...
    it("centres on a card and opens a file when the url names both", async () => {
      visit(
        "http://localhost/namespace-1?card=card-far&taskspace=taskspace-1&path=notes%2Ftodo.md",
      );
      vi.stubGlobal(
        "fetch",
      ...
      const { container } = render(NamespacePage, {
        props: { data: withTaskspace(), params: { namespaceId: "namespace-1" }, form: null },
      });
    ...
    it("leaves a file link inert in an export that carries no files", async () => {
      visit("http://localhost/namespace-1?taskspace=taskspace-1&path=notes%2Ftodo.md");
      const fetch = vi.fn();
      vi.stubGlobal("fetch", fetch);
...
describe("Squash", () => {
  ...
  it("replaces the selected card with the pieces the server answers with", async () => {
    ...
    vi.stubGlobal("fetch", fetch);
    render(NamespacePage, {
      props: { data: splittable, params: { namespaceId: "namespace-1" }, form: null },
    });
    ...
    expect(fetch).toHaveBeenCalledWith("/namespace-1/api/cards/squash", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
describe("Scoped-file export", () => {
  ...
  function exportData(suffix: string) {
    ...
    return {
      ...data,
      readonly: true,
      namespace: { id: `namespace-${suffix}`, name: `Namespace ${suffix}`, isDefault: false },
      cards: [],
      warps: [],
      warpDirectory: [],
  ...
  it("browses the arriving namespace's files after warping, not the one it was opened on", async () => {
    vi.stubGlobal("fetch", vi.fn());
    ...
    // And it is the arriving namespace's tree behind the row, not a leftover.
